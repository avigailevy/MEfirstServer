...
//פונקציה למציאת ID של קובץ על פי שם ותיקיית אב
async function findFileIdByName(fileName, parentFolderId = null) {
  const drive = google.drive({ version: 'v3', auth: await auth.getClient() });

  let q = `name='${fileName}' and mimeType!='application/vnd.google-apps.folder' and trashed=false`;
  if (parentFolderId) {
  ...
  if (res.data.files.length === 0) {
    return null; // לא נמצא קובץ כזה
  }

  return res.data.files[0].id; // מחזיר את ה-id של הקובץ הראשון שנמצא
}
